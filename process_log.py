...
try:
    # 从标准输入逐行读取日志
    print('epoch', '\t', 'scene', '\t', 'val_loss', '\t', 'ratio_type', '\t', 'click_ratio', '\t', 'like_ratio', '\t', 'comment_auc', '\t', '5s_ratio', '\t', '10s_ratio', '\t', '18s_ratio')
    for line in sys.stdin:
        ...
        if match_auc:
            ...
            # 格式化输出
            print(f"{epoch}\t{scene}\t{val_loss:.6f}\t{'auc'}\t{click_auc:.6f}\t{like_auc:.6f}\t{comment_auc:.6f}\t{five_s_auc:.6f}\t{ten_s_auc:.6f}\t{eighteen_s_auc:.6f}")
        if match_ndcg:
                        # 提取匹配数据
            ...
            # 格式化输出
            print(f"{epoch}\t{scene}\t{val_loss:.6f}\t{'ndcg'}\t{click_ndcg:.6f}\t{like_ndcg:.6f}\t{comment_ndcg:.6f}\t{five_s_ndcg:.6f}\t{ten_s_ndcg:.6f}\t{eighteen_s_ndcg:.6f}")
